...
def run(args):
    ...
    if args.test_only:
        confusion_meter, test_accuracy, failed_paths = test(test_data_loader, model, args.device, args)
...
def inference(frames, model, device):
...
def load_json(path, args):  # load keypoints from json
    json_data = json.load(open(path))
    # keypoints = np.zeros((1, args.time_window, 3, args.num_keypoints))
    keypoints = []
    for i, poses_frame in enumerate(json_data):
        try:
        ...
        assert hand_keypoints.shape == (3, args.num_keypoints)  # [C, V]
...
if __name__ == '__main__':
    ...
    sample_path = '/scratch/ychen855/hand_gesture/st_gcn/toyTestDataset/json_data/IMG_0617_draw-cross_1.json'

    keypoints = load_json(sample_path, args)
    all_frames = torch.from_numpy(keypoints).to(args.device)

    video_len = all_frames.shape[0]

    for i in range(video_len - args.time_window + 1):
        cur_frames = all_frames[i:i+args.time_window, :, :].unsqueeze(0)
        results = model(cur_frames)
        print(results)
        exit()
