...
def inference(model, images, image_size, conf_thres, nms_thres):
    detections = []
    for f in os.listdir(args.images):
        image_id = f[:-4]
        image = np.array(
            Image.open(os.path.join(args.images, f)).convert('RGB'),
            dtype=np.uint8)

        detection = detect_image(model, image, img_size=img_size, conf_thres=conf_thres, nms_thres=nms_thres)
        detections.append(detection)
    
    return detections

def inference_one(model, image, img_size, conf_thres, nms_thres):
